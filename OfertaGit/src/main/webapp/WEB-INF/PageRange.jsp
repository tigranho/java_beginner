                ...
                <div class="card">
                    ...
                    <div class="body">
                        <table id="mainTable" class="table table-striped">
                            ...
                            <tr>
                                <th>Արժույթ</th>
                                <th>Նվազագույն գումարը</th>
                                <th>Առավելագույն գումար</th>
                                <th>խոշորացում քայլերը</th>
                                <th>Թարմացնել</th>
                            </tr>
                            </thead>

                            <tbody>
                            <c:if test="${requestScope.dropDownsList != null}">
                                <c:forEach items="${requestScope.dropDownsList}" var="drop" varStatus="loop">
                                    <form action="UpdateRangeController" method="post">
                                        <tr>
                                            <td><c:out value="${drop.currancy}"/></td>
                                            <td><input type="number" name="minAmount" class="form-control"
                                                       value="${drop.minAmount}"/></td>
                                            <td><input type="number" name="maxAmount" class="form-control"
                                                       value="${drop.maxAmount}"/></td>
                                            <td><input type="number" name="Steps" class="form-control"
                                                       value="${drop.steps}"/>

                                    </td>
                                    <input type="hidden" name="Id" value="${drop.id}"/>
                                    <input type="hidden" name="Position" value="${drop.position}"/>
                                            <input type="hidden" name="currancy" value="${drop.currancy}"/>
                                    <td>
                                        <button type="submit"
