...
@WebServlet("/UpdateBlogImageInData")
@MultipartConfig(fileSizeThreshold=1024*1024*2, // 2MB
        maxFileSize=1024*1024*10,      // 10MB
        maxRequestSize=1024*1024*50)   // 50MB
public class UpdateBlogImageInData extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            updateBlogImageInData(request,response);
    ...
    private void updateBlogImageInData(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException, SQLException {
    ...
    private void UploadImageToServer(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException, SQLException {
        ...
        for (Part part : request.getParts()) {
            ...
            UpdateImageOfBlog(request,response,filepat,UAId);
        }

    }

    private void UpdateImageOfBlog(HttpServletRequest request, HttpServletResponse response, String filepat, int uaId) throws ServletException, IOException, SQLException {
        String linkToSave =  "http://www.oferta.am/" +filepat;
        UsefulArticles usefulArticles = new UsefulArticles(linkToSave);
        int UpdateImageInData = usefullArticlesDaoController.AddNewUsefulArticleImage(usefulArticles,UAId);
        if (UpdateImageInData == 0) {
            ...
            String message = "Բլոգի պատկերը հաջողությամբ թարմացնելու համար";
            BuildRequestToPage(request);
            goBackToSamePage(request, response, message);
        }
    }
