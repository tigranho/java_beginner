...
public class DeleteCard extends HttpServlet {
    ...
    private void deleteCard(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException, SQLException {
        request.setCharacterEncoding("UTF-8");
        sessionControlling(request, response);
        getParameters(request);
        getAdminInfo(request, response);
        deleteCardFromData(cardId, request, response);

    }

    private void deleteCardFromData(int cardId, HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException, SQLException {
        int deletCard = deleteCard(cardId);
        if(deletCard > 0) {
            getAllCards();
    ...
    private void getAllCards() throws SQLException {
        cardsList = cardsDao.getAllCardsList();
    }

    private int deleteCard(int cardId) throws SQLException {
