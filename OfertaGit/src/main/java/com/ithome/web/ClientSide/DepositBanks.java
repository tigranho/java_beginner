...
@WebServlet("/DepositBanks")
public class DepositBanks extends HttpServlet {
    ...
    private int bankID = 0;
    private List<Deposit> deposiOposit = new ArrayList<>();
    ...
    private void depositBanks(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException, SQLException {
    ...
    private void getOtherBanks() throws SQLException {
        List<Deposit> depoposit =new ArrayList<>();
        deposiOposit = new ArrayList<>();
        depoposit = depositDaoController.getAllDepositeList();
   ...
   /* private void getBanks() throws SQLException {
    ...
    private int getMinAmount() {
        for (int i = 0; i <depositList.size() ; i++) {
            MinAmount = depositList.get(i).getDMinAmount();
        }
        return MinAmount;
    }
    ...
    private void gotoBanksPage(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        request.getRequestDispatcher("/BankingBank.jsp").forward(request, response);
    }
   ...
   /* private void depositeFilterSub() throws SQLException {
        depositList = depositDaoController.getDepositMaxMinAmount(getMinAmount(), pageCurrancy);
    ...
    private void AmountAsecSub() throws SQLException {
        depositList = depositDaoController.getDepositMaxMinAmountSub(getMinAmount(), pageCurrancy);
    ...
    private void PercentageDescSub() throws SQLException {
        depositList = depositDaoController.getDepositMaxMinPercentSubDes(getMinAmount(), pageCurrancy);
    ...
    private void PercentageAsecSub() throws SQLException {
        depositList = depositDaoController.getDepositMaxMinPercentSubAsc(getMinAmount(), pageCurrancy);
