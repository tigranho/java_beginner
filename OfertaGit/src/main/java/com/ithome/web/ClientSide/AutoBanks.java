...
@WebServlet("/AutoBanks")
public class AutoBanks extends HttpServlet {
    ...
    private List<CarLoans> depositList = new ArrayList<>();
    ...
    private int bankID = 0;
    private List<CarLoans> deposiOposit = new ArrayList<>();
    ...
    CarLoanDao depositDaoController = new CarLoanDao();
    ...
    private void autoBanks(HttpServletRequest request, HttpServletResponse response) throws SQLException, IOException, ServletException {
    ...
    private void gotoBanksPage(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        request.getRequestDispatcher("/CaBankingBank.jsp").forward(request, response);
    }
    ...
    private void getOtherBanks() throws SQLException {
        List<CarLoans> depoposit = new ArrayList<>();
        deposiOposit = new ArrayList<>();
        depoposit = depositDaoController.getAllCarLoans();
    ...
    private int getMinAmount() {
        for (int i = 0; i <depositList.size() ; i++) {
            MinAmount = depositList.get(i).getCLMinAmount();
        }
        return MinAmount;
    }
    ...
    class DepositComaratorAmount implements Comparator<CarLoans> {
        ...
        @Override
        public int compare(CarLoans o1, CarLoans o2) {
            return Integer.compare(o2.getCLMinAmount(), o1.getCLMinAmount());
        }
    }

    private void AmountAsec() throws SQLException {
        depositList = depositDaoController.getDepositbyMaxMinAmountAsec(getMinAmount(),5000000,  pageCurrancy);
