...
@WebServlet("/CardBanks")
public class CardBanks extends HttpServlet {
    ...
    private List<Cards> depositList = new ArrayList<>();
    ...
    private int bankID = 0;
    private List<Cards> deposiOposit = new ArrayList<>();
    ...
    private void cardBanks(HttpServletRequest request, HttpServletResponse response) throws SQLException, IOException, ServletException {
    ...
    private void gotoBanksPage(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        request.getRequestDispatcher("/CaDBankingBank.jsp").forward(request, response);
    }
    ...
    private void getOtherBanks() throws SQLException {
        List<Cards> depoposit = new ArrayList<>();
        deposiOposit = new ArrayList<>();
        depoposit = depositDaoController.getAllCardsList();
    ...
    private int getMinAmount() {
        for (int i = 0; i <depositList.size() ; i++) {
            MinAmount = depositList.get(i).getCardMinCreditLimit();
        }
        return MinAmount;
    }
    ...
    private void depositeFilter() throws SQLException {
        depositList = depositDaoController.getDepositbyMaxMinAmountById(getMinAmount(),  pageCurrancy,Bankid);
        arrow = 0;
        arrow2 = 0;
    }
    ...
    private void PercentageDesc() throws SQLException {
      /*  // depositAllInSubRage = depositDaoController.getSemiDepositbyMaxMinAmount(Integer.parseInt(amoutFiltered), Integer.parseInt(MaxAmount),pageCurrancy);
        ...
        depositList = depositDaoController.getDepositMaxMinPercentByID(getMinAmount(),  pageCurrancy,Bankid);
    ...
    private void PercentageAsec() throws SQLException {
        depositList = depositDaoController.getDepositMaxMinPercentSubAcs(getMinAmount(),  pageCurrancy);
        Collections.sort(depositList, new DepositComarator());
        arrow = 1;
        arrow2 = 1;
    }

    class DepositComarator implements Comparator<Cards> {

        @Override
        public int compare(Cards o1, Cards o2) {
            return Double.compare(o2.getCardMinCreditLimit(), o1.getCardMinCreditLimit());
        }
    }

    class DepositComaratorAmount implements Comparator<Cards> {

        @Override
        public int compare(Cards o1, Cards o2) {
            return Double.compare(o2.getMinCashBack(), o1.getMinCashBack());
        }
    }
