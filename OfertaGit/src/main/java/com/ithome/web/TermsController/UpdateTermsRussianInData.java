...
@WebServlet("/UpdateTermsRussianInData")
public class UpdateTermsRussianInData extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        try {
            updateTermsRussianInData(request,response);
    ...
    private void updateTermsRussianInData(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException, SQLException {
        request.setCharacterEncoding("UTF-8");
        sessionControlling(request, response);
        getAdminInfo(request, response);
        getParameters(request);
        UpdateTermsInDataRus(CreateNewTermsInData(1),request,response,1);
    }


    private void UpdateTermsInDataRus(int createNewTermsInData, HttpServletRequest request, HttpServletResponse response, int id) throws ServletException, IOException, SQLException {
    ...
    private void goBackToErrorPage(HttpServletRequest request, HttpServletResponse response, String message) throws ServletException, IOException {
        request.setAttribute("message", message);
        request.getRequestDispatcher("/WEB-INF/TermsRussian.jsp").forward(request, response);
    }
    ...
    private void getRussianTerms() throws SQLException {
